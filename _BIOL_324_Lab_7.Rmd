...
First let's install the required packages. If you have an old"er version of R, you may need to install them individually, e.g. by running `install.packages("mapr")` and then the next packagage, and so on. 
...
These packages are designed for connecting with the GBIF database (`spocc`, `taxize`), organizing data (`tidyverse`), and mapping (`mapr`). Once installed, use the `library()` function to attach the packages:
